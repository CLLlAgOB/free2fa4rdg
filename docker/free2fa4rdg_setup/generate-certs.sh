...
generate_service_cert() {
    ...
    log "INFO" "[req]" >"$SERVICE_CONFIG"
    log "INFO" "distinguished_name = req_distinguished_name" >>"$SERVICE_CONFIG"
    log "INFO" "req_extensions = v3_req" >>"$SERVICE_CONFIG"
    log "INFO" "prompt = no" >>"$SERVICE_CONFIG"
    log "INFO" "" >>"$SERVICE_CONFIG"
    log "INFO" "[req_distinguished_name]" >>"$SERVICE_CONFIG"
    log "INFO" "CN = $SERVICE" >>"$SERVICE_CONFIG"
    log "INFO" "" >>"$SERVICE_CONFIG"
    log "INFO" "[v3_req]" >>"$SERVICE_CONFIG"
    log "INFO" "keyUsage = digitalSignature, keyEncipherment, keyAgreement" >>"$SERVICE_CONFIG"
    log "INFO" "extendedKeyUsage = serverAuth, clientAuth" >>"$SERVICE_CONFIG"
    log "INFO" "subjectAltName = @alt_names" >>"$SERVICE_CONFIG"
    log "INFO" "" >>"$SERVICE_CONFIG"
    log "INFO" "[alt_names]" >>"$SERVICE_CONFIG"
    log "INFO" "DNS.1 = $SERVICE" >>"$SERVICE_CONFIG"
    if [ ! -z "$ADDITIONAL_DNS_NAME_FOR_ADMIN_HTML" ]; then
        log "INFO" "DNS.2 = $ADDITIONAL_DNS_NAME_FOR_ADMIN_HTML" >>"$SERVICE_CONFIG"
    fi
