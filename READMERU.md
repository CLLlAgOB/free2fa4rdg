...
- `FREE2FA_TELEGRAM_BOT_PROXY`: Необязательный прокси для запросов Telegram-бота.
Поддерживаются форматы `socks5://`, `socks5://user:password@host:port` и `http://`.
Если параметр пустой или не задан, прокси отключен и бот работает напрямую.
...
- Добавлена необязательная поддержка прокси для Telegram-бота.
- Добавлен новый параметр окружения: `FREE2FA_TELEGRAM_BOT_PROXY`.
- По умолчанию прокси отключен. Если параметр пустой или не задан, бот работает без прокси.
- Улучшена обработка ошибок при старте бота: если указанный прокси недоступен, задача бота больше не завершается с необработанным исключением.
- Теперь приложение записывает предупреждение в лог о недоступности Telegram-прокси и продолжает периодически повторять попытку запуска бота.

Примеры:
- без прокси: оставить `FREE2FA_TELEGRAM_BOT_PROXY` пустым
- SOCKS5 прокси: `socks5://proxy-host:1080`
- SOCKS5 прокси с авторизацией: `socks5://user:password@proxy-host:1080`
- HTTP прокси: `http://proxy-host:3128`

Дополнительные действия для обновления с предыдущей версии:
Добавить новую переменную в файл `.env` и `docker-compose.yml`

В конец `.env` добавить:
```shell
